...
server.post("/google-auth", async (req, res) => {
    ...
    .then(async (decodedUser) => {
        ...
        if(user) {
            if(!user.google_auth){
                return res.status(403).json({"error": "This email was signed up without google. Please logging in with password to access the account!"})
            }
        }
        else{
            let username= await generateUsername(email);
...
server.post('/latest-blogs', async (req, res) => {
    ...
    Blog.find({draft: false}).sort({"publishedAt": -1}).skip(10 * (page - 1)).limit(10).populate('author', 'personal_info.username personal_info.fullname personal_info.profile_img -_id').select("blog_id title banner des tags publishedAt -_id activity.total_likes activity.total_reads")
    .then(blogs => {
        return res.status(200).json({ blogs });
    })
    .catch(err => {
...
server.post('/search-blogs', async (req, res) => {
  ...
  findQuery = {
    ...
    Blog.find(findQuery)
        ...
        .select("blog_id title banner des tags content publishedAt activity.total_likes activity.total_reads -_id ")
        .skip(20 * (page - 1))
        .then(blogs => {
            return res.status(200).json({ blogs });
        })
